...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const jobData = `
      ...
      WHERE jh.buyerId = ? AND NOT (COALESCE(jh.accept,-1) = 1 OR (jh.progress = 3 AND COALESCE(jh.accept,-1) = 0))
    `;
    ...
    const orderData = `
      ...
      FROM orders o
      LEFT JOIN users u ON o.sellerId = u.userId
      WHERE o.buyerId = ? AND o.completed = FALSE
    `;
